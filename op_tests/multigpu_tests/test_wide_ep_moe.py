...
def _run_one_bs(
    ...
    if not staged_only:
        ...
        assert torch.isfinite(
            out.float()
        ).all(), "TestWideEpMoe output has non-finite values"
    ...
    if bs <= accuracy_max_bs:
        ...
        if os.environ.get("AITER_DEBUG_MX_EP_SYNC", "0") == "1":
            ...
            if debug_sort is not None and stage1_call is not None:
                ...
                for expert_id in torch.unique(expert_rows[valid]).tolist():
                    ...
                    byte_addr = (
                        (chunk * per_chunk_dw + ku * 64 + wave_group * 16 + m_lane) * 4
                        + ikxdl * 2
                        + row_half
                    )
                    scale_raw = stage1_scale.view(torch.uint8).flatten()[byte_addr]
                    got_f32 = fp4_utils.mxfp4_to_f32(got).view(rows.numel(), inter_dim)
        ...
        assert torch.isfinite(
            diagnostic_out.float()
        ).all(), "combine output has non-finite values"
    ...
    moe_tflops_best = (
        _moe_flops(local_hits_max) / (gemm_total_min_us * 1e6)
        if gemm_total_min_us > 0
        else 0.0
    )
    moe_tflops_worst = (
        _moe_flops(local_hits_max) / (gemm_total_max_us * 1e6)
        if gemm_total_max_us > 0
        else 0.0
    )
    moe_gbps = _gbps(
        _moe_bytes(total_recv_avg, active_experts_avg), gemm_total_us / 1000
    )
    moe_gbps_best = _gbps(
        _moe_bytes(total_recv_avg, active_experts_avg), gemm_total_min_us / 1000
    )
    moe_gbps_worst = _gbps(
        _moe_bytes(total_recv_avg, active_experts_avg), gemm_total_max_us / 1000
    )
